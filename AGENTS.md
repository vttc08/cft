...
5. **Use Termux's packaged Python runtime on Android.**
   - Keep `uv` configured to prefer the system interpreter so Termux-native standard-library
     extensions link against the libraries installed under `$PREFIX`.
   - Treat `pyexpat` as part of CPython plus Termux's `libexpat`, not as a PyPI dependency.
   - Keep missing Python XML support distinct from AWS credential failures in TUI guidance.
...
7. **Separate operational usage from billing truth.**
